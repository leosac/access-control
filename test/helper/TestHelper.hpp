...
{
/**
* Helper function that create an object of type ModuleType (using conventional
* parameter) and run it.
* This runs in "module"'s thread.
*/
template <typename ModuleType>
bool test_run_module(zmqpp::context *ctx, zmqpp::socket *pipe,
                     const boost::property_tree::ptree &cfg)
{
    {
        CoreUtilsPtr u = std::make_shared<CoreUtils>(
            nullptr, nullptr, std::make_shared<ConfigChecker>(), false);
...
bus_read_extract(zmqpp::message *m, T first_arg, Content... content)
{
    T value;
    *m >> value;
    if (value != first_arg)
...
/**
* Base class for test fixtures, it defines a ZMQ context, a BUS and a sub socket
* connect to the bus.
*/
class TestHelper : public ::testing::Test
{
  ...
  public:
    ...
    /**
    * We need this 2-step initialization to prevent calling virtual method
    * (run_module) in constructor.
    */
    virtual void SetUp() override final
    {
        module_actor_ = std::unique_ptr<zmqpp::actor>(new zmqpp::actor(
            std::bind(&TestHelper::run_module, this, std::placeholders::_1)));
    }
