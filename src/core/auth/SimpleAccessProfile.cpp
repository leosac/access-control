...
bool SimpleAccessProfile::isAccessGranted(const std::chrono::system_clock::time_point &date, AuthTargetPtr target)
{
    ...
    for (const auto &time_frame : time_frames_[target->name()])
    {
        if (time_frame.day != time_out->tm_wday)
            continue;
        if (!(time_frame.start_hour < time_out->tm_hour ||
                (time_frame.start_hour == time_out->tm_hour && time_frame.start_min <= time_out->tm_min)))
            continue;
        if (!(time_frame.end_hour > time_out->tm_hour ||
                (time_frame.end_hour == time_out->tm_hour && time_frame.end_min >= time_out->tm_min)))
            continue;
        return true;
    }
    return false;
