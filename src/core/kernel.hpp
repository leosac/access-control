...
* This will destroy the kernel object, unload all module, and restart.
* RESET                    |                     |                  | Reset Leosac
* configuration to factory default. This will also restart.
* GET_NETCONFIG            |                     |                  | Send the
* network config.
* SET_NETCONFIG            | Serialized ptree    |                  | Write the new
* network config to file
* SCRIPTS_DIR              |                     |                  | Ask the path to
* scripts directory
* FACTORY_CONFIG_DIR       |                     |                  | Ask the path to
* factory config directory
*
*
* ### Notifications
* Leosac core occasionally broadcast notification for other subsystem. The message's
* topic
* is "KERNEL" for all notification coming from Leosac's core.
*     + `SYSTEM_READY` message when all modules are initialized.
*     + `SIGHUP` to tell module to reload their configuration (if they support
* hot-reload).
...
class Kernel
{
  public:
    ...
    */
    ...
    make_config(const Leosac::Tools::RuntimeOptions &opt);

    /**
    * Main loop of the main thread.
    * What it does:
    *      1. It starts the communication BUS for devices's abstraction and general
    * module to use.
    *      2. It starts by loading modules requested in configuration
    *      3. Wait for shutdown / restart order (linux signals).
    * @return If it returns true that means the application should restart instead of
    * stopping.
    */
    bool run();
    ...
    * Uses environment variable if available, otherwise defaults to hardcoded path
    * relative to cwd.
    */
    ...
    * This means that configuration change made when Leosac was running will be
    * persisted.
    */
    bool save_config();
  ...
  private:
    ...
    */
    ...
    /**
    * Manages the different libraries (.so) we load, path to those libraries, modules
    * instantiation.
    */
